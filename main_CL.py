...
sys.path.append(sys.path[0].split("/e_cl")[0])
print(sys.path)
...
def main_cl(params):
    ...
    if 'bert-base-cased' in params.model_name or 'bert-base-chinese' in params.model_name:
    ...
    for iteration in range(total_iter): # 遍历每个任务
        ...
        # params.is_load_common_first_model = False
        ...
        if params.is_from_scratch or iteration == 0:
            ...
            if 'bert-base-cased' in params.model_name or 'bert-base-chinese' in params.model_name:
                ...
                raise Exception('model name %s is invalid'%params.model_name)
            trainer.model = model
            trainer.model.to(params.device)
        ...
        if iteration>0 and params.is_rescale_new_weight and (not params.is_from_scratch):   # True
            ...
            trainer.model.classifier.fc2.weight.data = rescale_weight_matrix.type(torch.FloatTensor).to(params.device)
        ...
        if params.is_load_ckpt_if_exists: # True
            ...
            # print(iteration, params.is_load_common_first_model, common_first_model_ckpth_path, os.path.isfile(common_first_model_ckpth_path))
            # 跳过第一个任务/base的训练
        ...
        for e in range(1, training_epochs+1):
            ...
            loss_list, distill_list, ce_list, kl_list, sim_list = [], [], [], [], []
            # average loss
            mean_loss = 0.0
            # training acc
            ...
            for X, y in bar:
                ...
                #     print(auto_tokenizer.convert_ids_to_tokens(X[0]))
                #     print(y[0])
                #
                # exit(0)
                ...
                # Compute loss
                if iteration>0:
                    ce_loss, distill_loss, kl_loss, sim_loss = trainer.batch_loss_ede(y)
                    ce_list.append(ce_loss)
                    distill_list.append(distill_loss)
                    kl_list.append(kl_loss)
                    sim_list.append(sim_loss)
                ...
                mean_kl_loss = np.mean(kl_list) if len(kl_list)>0 else 0 # 平均每个batch的kl loss
                mean_sim_loss = np.mean(sim_list) if len(sim_list)>0 else 0 # 平均每个batch的sim loss
           

                # Print training information
                if params.info_per_steps>0 and step%params.info_per_steps==0: # params.info_per_steps=0
                    logger.info("Epoch %d, Step %d: Total_loss=%.3f, CE_loss=%.3f, Distill_loss=%.3f, kl=%.4f, sim=%.4f, Training_exact_match=%.2f%%"%(
                            e, step, mean_loss, \
                            mean_ce_loss, mean_distill_loss, mean_kl_loss, mean_sim_loss, correct_cnt/total_cnt*100
                    ))
                    # reset the loss lst
                    loss_list = []
                    distill_list = []
            ...
            if params.info_per_epochs>0 and e%params.info_per_epochs==0: # params.info_per_epochs=1    每隔一个epoch 输出信息s
                logger.info(
                    "Epoch %d, Step %d: Total_loss=%.3f, CE_loss=%.3f, Distill_loss=%.3f, kl=%.6f, sim=%.6f, Training_exact_match=%.2f%%" % (
                        e, step, mean_loss, \
                        mean_ce_loss, mean_distill_loss, mean_kl_loss, mean_sim_loss, correct_cnt / total_cnt * 100
                    ))
            # Update lr + save skpt + do evaluation
        ...
        # trainer.entropy_plot(dataloader_test_cumul)
        # exit(0)
        ...
        #     f.write(str(params.exp_id) + str(f1_test_cumul) + "\n")
        #
        #
        #
        #
